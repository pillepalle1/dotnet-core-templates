...
{
    public class StartHandler : AUpdateHandler<UpdateHandlerArgs>
    {
        ...
        protected override async Task HandlerLogicImpl(UpdateHandlerArgs args)
        {
            ...
            Message m = args.MessageHandlerArgs.Message;

            // We only process text-messages
            if (m.Type == MessageType.Text)
            {
                if (args.MessageHandlerArgs.CommandTokens[0].ToLower().Equals("/start"))
                {
